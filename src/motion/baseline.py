...
def ddpg(n_episodes, print_every, max_t, score_solved, param, CONFIG_PATH, useful):
     ...
     if param["TYPE"] == 0:
          ...
          scores_window = (
               deque()
          )  # average scores of the most recent episodes                                                     # list of average scores of each episode
          ...
          for i_episode in range(n_episodes + 1):  # initialize score for each agent
               ...
               for t in range(max_t):
                    ...
                    next_states, rewards, done, _ = env.step_env(
                         actions
               ...
               if i_episode % 300 == 0:
                    torch.save(
                         ...
                         os.path.join(
                         checkpoints_dir, "{}_actor_checkpoint.pth".format(i_episode)
                         ),
                    )
                    torch.save(
                         agent.critic_local.state_dict(),
                         os.path.join(
                         checkpoints_dir, "{}_critic_checkpoint.pth".format(i_episode)
                         ),
                    )
               ...
               if np.mean(scores_window) >= score_solved:
                    ...
                    torch.save(
                         agent.actor_local.state_dict(),
                         os.path.join(checkpoints_dir, "actor_checkpoint.pth"),
                    )
                    torch.save(
                         agent.critic_local.state_dict(),
                         os.path.join(checkpoints_dir, "critic_checkpoint.pth"),
                    )
                    break
     ...
     if param["TYPE"] == 1:
          ...
          agent.actor_local.load_state_dict(
               torch.load(param["MODEL"] + "actor_model.pth")
          )
          agent.critic_local.load_state_dict(
               torch.load(param["MODEL"] + "critic_model.pth")
          )
