...
class Extension():
     ...
     def poses(self, path_waypoints):
          ...
          with open('/home/robofei/ws/src/vault/config/pose/' + path_waypoints) as f:
     ...
     def load_config(self, config_name):
          with open(os.path.join('/home/robofei/ws/src/vault/config/config.yaml')) as file:
     ...
     def save_results(self, type, result):
          ...
          if result == None:
               result = 0
          if type == "reward":
               file = open('/home/robofei/ws/src/vault/src/vault/run/' + 'Reward.txt', 'a')
               file.write("%f\n" % (np.mean(result)))

          if type == "loss":
               file = open('/home/robofei/ws/src/vault/src/vault/run/' + 'Loss.txt', 'a')
               file.write("%f\n" % (result.item()))

          if type == "Av":
               file = open('/home/robofei/ws/src/vault/src/vault/run/' + 'AvQ.txt', 'a')
               file.write("%f\n" % (result.item()))
          
          if type == "Max":
               file = open('/home/robofei/ws/src/vault/src/vault/run/' + 'MaxQ.txt', 'a')
               file.write("%f\n" % (result.item()))
          
          if type == "episode":
               file = open('/home/robofei/ws/src/vault/src/vault/run/' + 'Episode.txt', 'a')
               file.write("%f\n" % int((result)))
